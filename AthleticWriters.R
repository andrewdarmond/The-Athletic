...
Ken_Rosenthal_df <- data_frame(line=5:22, text=Ken_Rosenthal)

Ken_Rosenthal_tokens <- Ken_Rosenthal_df %>%
...
Joe_Posnanski_df <- data_frame(line=35:68, text=Joe_Posnanski)

Joe_Posnanski_tokens <- Joe_Posnanski_df %>%
  unnest_tokens(word, text) %>%
  anti_join(stop_words) %>%
  count(word, sort=T) #%>%
  #inner_join(get_sentiments("afinn")) %>%
...
Daniel_Popper_df <- data_frame(line=80:92, text=Daniel_Popper)

Daniel_Popper_tokens <- Daniel_Popper_df %>%
  unnest_tokens(word, text) %>%
  anti_join(stop_words) %>%
  count(word, sort=T)  #%>%
  #inner_join(get_sentiments("afinn")) %>%
...
Johnson_Twomey_tokens <- Johnson_Twomey_df %>%
  unnest_tokens(word, text) %>%
  anti_join(stop_words) %>%
  count(word, sort=T) # %>%
  #inner_join(get_sentiments("afinn")) %>%
...
Bill_Shea_df <- data_frame(line=150:199, text=Bill_Shea)

Bill_Shea_tokens <- Bill_Shea_df %>%
  unnest_tokens(word, text) %>%
  anti_join(stop_words) %>%
  count(word, sort=T)  #%>%
  #inner_join(get_sentiments("afinn")) %>%
...
frequency <- bind_rows(mutate(Ken_Rosenthal_tokens, author="Ken_Rosenthal"),
                       mutate(Joe_Posnanski_tokens, author= "Joe_Posnanski"),
                       mutate(Daniel_Popper_tokens, author="Daniel_Popper"),
                       mutate(Johnson_Twomey_tokens, author="Johnson_Twomey"),
                       mutate(Bill_Shea_tokens, author="Bill_Shea")
)%>%#closing bind_rows
  ...
  gather(author, proportion,`Joe_Posnanski`,`Daniel_Popper`,`Johnson_Twomey`,`Bill_Shea`)

#let's plot the correlograms:
ggplot(frequency, aes(x=proportion, y=`Ken_Rosenthal`, 
                      color = abs(`Ken_Rosenthal`- proportion)))+
  ...
  labs(y= "Ken_Rosenthal", x=NULL)

##########################################
##doing the cor.test() ################
##########################################

cor.test(data=frequency[frequency$author == "Joe_Posnanski",],
         ~proportion + `Ken_Rosenthal`)

cor.test(data=frequency[frequency$author == "Daniel_Popper",],
         ~proportion + `Ken_Rosenthal`)

cor.test(data=frequency[frequency$author == "Johnson_Twomey",],
         ~proportion + `Ken_Rosenthal`)

cor.test(data=frequency[frequency$author == "Bill_Shea",],
         ~proportion + `Ken_Rosenthal`)
...
Ken_Rosenthal_bigrams <- Ken_Rosenthal_df %>%
  unnest_tokens(bigram, text, token = "ngrams", n=2)

Ken_Rosenthal_bigrams #We want to see the bigrams (words that appear together, "pairs")

Ken_Rosenthal_bigrams %>%
  count(bigram, sort = TRUE) #this has many stop words, need to remove them 

#to remove stop words from the bigram data, we need to use the separate function:
Ken_Rosenthal_separated <- Ken_Rosenthal_bigrams %>%
  separate(bigram, c("word1", "word2"), sep = " ")

Ken_Rosenthal_filtered <- Ken_Rosenthal_separated %>%
  filter(!word1 %in% stop_words$word) %>%
  filter(!word2 %in% stop_words$word)

#creating the new bigram, "no-stop-words":
Ken_Rosenthal_counts <- Ken_Rosenthal_filtered %>%
...
###########################################################
Ken_Rosenthal_quadrogram <- Ken_Rosenthal_df %>%
...
Ken_Rosenthal_quadrogram

######################################################
####### VISUALISING A BIGRAM NETWORK #################
######################################################

Ken_Rosenthal_bigram_graph <- Ken_Rosenthal_counts %>%
  filter(n>20) %>%
  graph_from_data_frame()

Ken_Rosenthal_bigram_graph

ggraph(Ken_Rosenthal_bigram_graph, layout = "fr") +
  geom_edge_link()+
  geom_node_point()+
...
Joe_Posnanski_bigrams <- Joe_Posnanski_df %>%
  unnest_tokens(bigram, text, token = "ngrams", n=2)

Joe_Posnanski_bigrams #We want to see the bigrams (words that appear together, "pairs")

Joe_Posnanski_bigrams %>%
  count(bigram, sort = TRUE) #this has many stop words, need to remove them 

#to remove stop words from the bigram data, we need to use the separate function:
Joe_Posnanski_separated <- Joe_Posnanski_bigrams %>%
  separate(bigram, c("word1", "word2"), sep = " ")

Joe_Posnanski_filtered <- Joe_Posnanski_separated %>%
  filter(!word1 %in% stop_words$word) %>%
  filter(!word2 %in% stop_words$word)

#creating the new bigram, "no-stop-words":
Joe_Posnanski_counts <- Joe_Posnanski_filtered %>%
  count(word1, word2, sort = TRUE)
#want to see the new bigrams
...
################ 4 consecutive words - quadro-gram ########
###########################################################
Joe_Posnanski_quadrogram <- Joe_Posnanski_df %>%
  unnest_tokens(quadrogram, text, token = "ngrams", n=4) %>%
  separate(quadrogram, c("word1", "word2", "word3", "word4"), sep=" ") %>%
  filter(!word1 %in% stop_words$word) %>%
  filter(!word2 %in% stop_words$word) %>%
...
Joe_Posnanski_bigram_graph <- Joe_Posnanski_counts %>%
  filter(n>20) %>%
  graph_from_data_frame()

Joe_Posnanski_bigram_graph

ggraph(Joe_Posnanski_bigram_graph, layout = "fr") +
  geom_edge_link()+
  geom_node_point()+
  geom_node_text(aes(label=name), vjust =1, hjust=1)
...
Daniel_Popper_bigrams <- Daniel_Popper_df %>%
  unnest_tokens(bigram, text, token = "ngrams", n=2)

Daniel_Popper_bigrams #We want to see the bigrams (words that appear together, "pairs")

Daniel_Popper_bigrams %>%
  count(bigram, sort = TRUE) #this has many stop words, need to remove them 

#to remove stop words from the bigram data, we need to use the separate function:
library(tidyr)
Daniel_Popper_separated <- Daniel_Popper_bigrams %>%
  separate(bigram, c("word1", "word2"), sep = " ")

Daniel_Popper_filtered <- Daniel_Popper_separated %>%
  filter(!word1 %in% stop_words$word) %>%
  filter(!word2 %in% stop_words$word)

#creating the new bigram, "no-stop-words":
Daniel_Popper_counts <- Daniel_Popper_filtered %>%
  count(word1, word2, sort = TRUE)
#want to see the new bigrams
...
################ 4 consecutive words - quadro-gram ########
###########################################################
Daniel_Popper_quadrogram <- Daniel_Popper_df %>%
  unnest_tokens(quadrogram, text, token = "ngrams", n=4) %>%
  separate(quadrogram, c("word1", "word2", "word3", "word4"), sep=" ") %>%
  filter(!word1 %in% stop_words$word) %>%
  filter(!word2 %in% stop_words$word) %>%
...
Daniel_Popper_bigram_graph <- Daniel_Popper_counts %>%
  filter(n>20) %>%
  graph_from_data_frame()

Daniel_Popper_bigram_graph

ggraph(Daniel_Popper_bigram_graph, layout = "fr") +
  geom_edge_link()+
  geom_node_point()+
  geom_node_text(aes(label=name), vjust =1, hjust=1)
...
Johnson_Twomey_bigrams <- Johnson_Twomey_df %>%
  unnest_tokens(bigram, text, token = "ngrams", n=2)

Johnson_Twomey_bigrams #We want to see the bigrams (words that appear together, "pairs")

Johnson_Twomey_bigrams %>%
  count(bigram, sort = TRUE) #this has many stop words, need to remove them 

#to remove stop words from the bigram data, we need to use the separate function:
library(tidyr)
Johnson_Twomey_separated <- Johnson_Twomey_bigrams %>%
  separate(bigram, c("word1", "word2"), sep = " ")

Johnson_Twomey_filtered <- Johnson_Twomey_separated %>%
  filter(!word1 %in% stop_words$word) %>%
  filter(!word2 %in% stop_words$word)

#creating the new bigram, "no-stop-words":
Johnson_Twomey_counts <- Johnson_Twomey_filtered %>%
  count(word1, word2, sort = TRUE)
#want to see the new bigrams
...
################ 4 consecutive words - quadro-gram ########
###########################################################
Johnson_Twomey_quadrogram <- Johnson_Twomey_df %>%
  unnest_tokens(quadrogram, text, token = "ngrams", n=4) %>%
  separate(quadrogram, c("word1", "word2", "word3", "word4"), sep=" ") %>%
  filter(!word1 %in% stop_words$word) %>%
  filter(!word2 %in% stop_words$word) %>%
...
Johnson_Twomey_quadrogram

######################################################
####### VISUALISING A BIGRAM NETWORK #################
######################################################

Johnson_Twomey_bigram_graph <- Johnson_Twomey_counts %>%
  filter(n>20) %>%
  graph_from_data_frame()

Johnson_Twomey_bigram_graph


ggraph(Johnson_Twomey_bigram_graph, layout = "fr") +
  geom_edge_link()+
  geom_node_point()+
  geom_node_text(aes(label=name), vjust =1, hjust=1)
...
Bill_Shea_bigrams <- Bill_Shea_df %>%
  unnest_tokens(bigram, text, token = "ngrams", n=2)

Bill_Shea_bigrams #We want to see the bigrams (words that appear together, "pairs")
...
#to remove stop words from the bigram data, we need to use the separate function:
library(tidyr)
Bill_Shea_separated <- Bill_Shea_bigrams %>%
  separate(bigram, c("word1", "word2"), sep = " ")

Bill_Shea_filtered <- Bill_Shea_separated %>%
  filter(!word1 %in% stop_words$word) %>%
  filter(!word2 %in% stop_words$word)

#creating the new bigram, "no-stop-words":
Bill_Shea_counts <- Bill_Shea_filtered %>%
  count(word1, word2, sort = TRUE)
#want to see the new bigrams
Bill_Shea_counts
...
################ 4 consecutive words - quadro-gram ########
###########################################################
Bill_Shea_quadrogram <- Bill_Shea_df %>%
  unnest_tokens(quadrogram, text, token = "ngrams", n=4) %>%
  separate(quadrogram, c("word1", "word2", "word3", "word4"), sep=" ") %>%
  filter(!word1 %in% stop_words$word) %>%
  filter(!word2 %in% stop_words$word) %>%
...
Bill_Shea_quadrogram

######################################################
####### VISUALISING A BIGRAM NETWORK #################
######################################################

Bill_Shea_bigram_graph <- Bill_Shea_counts %>%
  filter(n>20) %>%
  graph_from_data_frame()

Bill_Shea_bigram_graph

ggraph(Bill_Shea_bigram_graph, layout = "fr") +
  geom_edge_link()+
  geom_node_point()+
  geom_node_text(aes(label=name), vjust =1, hjust=1)
